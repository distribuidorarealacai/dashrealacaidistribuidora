...
function ra(abs,ini,fim){if(!abs||abs.length===0){document.getElementById('kpiA').innerHTML='<div class="nd">Nenhum abastecimento no periodo.</div>';document.getElementById('tbA').innerHTML='';if(cA)cA.destroy();if(cAD)cAD.destroy();return}var pv={},pl={},pd={};abs.forEach(function(a){var n=a.veiculo;var v=parseFloat(a.valor)||0;var l=parseFloat(a.litros)||0;if(!pv[n])pv[n]=0;pv[n]+=v;if(!pl[n])pl[n]=0;pl[n]+=l;if(!pd[a.data])pd[a.data]=0;pd[a.data]+=v});var nAbs=abs.length,nVec=Object.keys(pv).length,total=Object.entries(pv).reduce(function(s,e){return s+e[1]},0),totL=Object.entries(pl).reduce(function(s,e){return s+e[1]},0),dp=cd(ini,fim);document.getElementById('kpiA').innerHTML='<div class="kc tel"><div class="kl">Abastecimentos</div><div class="kv">'+nAbs+'</div><div class="ks">'+dp+' dia(s)</div></div><div class="kc"><div class="kl">Veículos</div><div class="kv">'+nVec+'</div><div class="ks">ativos</div></div><div class="kc amb"><div class="kl">Litros</div><div class="kv">'+totL.toFixed(1)+'</div><div class="ks">no periodo</div></div><div class="kc grn"><div class="kl">Média</div><div class="kv">'+fmt(total/nAbs)+'</div><div class="ks">por abastec.</div></div>';var x=document.getElementById('cA').getContext('2d');if(cA)cA.destroy();var eo=Object.entries(pv).sort(function(a,b){return b[1]-a[1]});cA=new Chart(x,{type:'bar',data:{labels:eo.map(function(x){return x[0]}),datasets:[{data:eo.map(function(x){return x[1]}),backgroundColor:eo.map(function(_,i){return C[i%C.length]+'cc'}),borderColor:eo.map(function(_,i){return C[i%C.length]}),borderWidth:2,borderRadius:6}]},options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{ticks:{callback:function(v){return fmt(v)}}}}});var x2=document.getElementById('cAD').getContext('2d');if(cAD)cAD.destroy();var tk=Object.keys(pd).sort(),dc=[],vd=[];tk.forEach(function(d){if(pd[d]>0){dc.push(d);vd.push(pd[d])}});var g=x2.createLinearGradient(0,0,0,320);g.addColorStop(0,'rgba(20,184,166,0.3)');g.addColorStop(1,'rgba(20,184,166,0.02)');cAD=new Chart(x2,{type:'line',data:{labels:dc.map(fd),datasets:[{data:vd,borderColor:'#14b8a6',backgroundColor:g,borderWidth:3,fill:true,tension:0.3,pointRadius:4,pointBackgroundColor:'#14b8a6'}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{ticks:{callback:function(v){return fmt(v)}}}}});var h='';Object.entries(pv).sort(function(a,b){return b[1]-a[1]}).forEach(function(entry,i){var n=entry[0],v=entry[1];var l=pl[n]||0;var p=total>0?(v/total*100):0;var c=C[i%C.length];h+='<tr><td class="vn"><span style="display:inline-block;width:12px;height:12px;border-radius:50%;background:'+c+';margin-right:8px"></span>'+n+'</td><td>'+l.toFixed(1)+' L</td><td>'+fmt(v)+'</td><td><span class="pb"><span class="pf" style="width:'+p+'%;background:'+c+'"></span></span>'+p.toFixed(1)+'%</td></tr>'});h+='<tr style="background:rgba(20,184,166,0.08);border-top:2px solid #14b8a6;font-weight:700"><td><strong>Total do período</strong></td><td>'+totL.toFixed(1)+' L</td><td><strong>'+fmt(total)+'</strong></td><td>100%</td></tr>';document.getElementById('tbA').innerHTML=h}
