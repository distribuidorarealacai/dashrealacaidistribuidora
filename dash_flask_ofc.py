...
def gerar_dashboard_html(pedidos, entregas):
    dj = base64.b64encode(json.dumps(pedidos, ensure_ascii=False).encode('utf-8')).decode('ascii')
    ej = base64.b64encode(json.dumps(entregas, ensure_ascii=False).encode('utf-8')).decode('ascii')
    with _metas_lock:
        mj = base64.b64encode(json.dumps(_metas, ensure_ascii=False).encode('utf-8')).decode('ascii')
...
const TP=JSON.parse(d64('__DJ__')),TE=JSON.parse(d64('__EJ__')),M=JSON.parse(d64('__MJ__')),MC=__MC__,C=['#3b82f6','#10b981','#f59e0b','#ef4444','#8b5cf6','#ec4899','#14b8a6','#f97316','#6366f1','#84cc16','#06b6d4','#a855f7'];
